...
class Cube{                 //魔方类
public:
    Cube():m_action(0){}        //构造函数
    Cube(const Cube &c);

    void inputCube(const std::string &filename);        //文件输入魔方起始状态
    bool operator==(const Cube &that) const;            
    bool isTarget() const;                              //判断是否解决
    void move(int act);                                 //18种移动方法
    ...
    char m_front[3][3];
    char m_back[3][3];
    int m_action;           //记录进入下一节点前搜索的分支序号，便于退回时继续搜索
};
...
void createChild(Cube* &child,Cube* &crntnode,int &act);        //创建子节点

void printAns(std::deque<ans_node> ans);            //输出结果

bool search(const Cube &c);              //搜索函数
...
void copy(Cube &target,Cube &source){
    for(int i=0;i<3;i++){
        for(int j=0;j<3;j++){
            ...
            target.m_back[i][j]=source.m_back[i][j];
        }
    }
}
